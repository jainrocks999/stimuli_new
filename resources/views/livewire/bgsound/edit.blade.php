<form wire:submit.prevent="submit" class="pt-3">
    ...
    <div class="form-group {{ $errors->has('mediaCollections.bgsound_sound') ? 'invalid' : '' }}">
        ...
        <x-dropzone id="sound" name="bgsound_sound" action="{{ route('admin.bgsounds.storeMedia') }}" collection-name="bgsound_sound" max-file-size="5000" max-files="1" />
        <div class="validation-message">
    ...
    <div class="form-group {{ $errors->has('bgsound.is_premium') ? 'invalid' : '' }}">
        ...
        @foreach($this->listsForFields['is_premium'] as $key => $value)
            <label class="radio-label"><input type="radio" name="is_premium" wire:model="bgsound.is_premium" value="{{ $key }}">{{ $value }}</label>
        @endforeach
        <div class="validation-message">
